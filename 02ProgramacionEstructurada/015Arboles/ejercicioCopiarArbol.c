...
struct Node *copiaRaiz;

//Funcion para copiar el arbol
struct Node *copiarArbol(struct Node *root, int contador){
    if(root == NULL){
        return NULL;
    }
    
    struct Node *copia = (struct Node *)
        malloc(sizeof(struct Node));
    ...
    copia->left = copiarArbol(root->left, contador);
    copia->right = copiarArbol(root->right, contador);
    printf("Imprimiendo copia raiz");
    imprimirArbol(copia, 0);
...
int main()
{
    ...
    //Copiar arbol
    struct Node *copia = copiarArbol(root, 0);
    imprimirArbol(copia, 0);
